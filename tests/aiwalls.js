...
(async () => {
  ...
  const bell = await page.evaluate(() => { const p = Game.players[1], th = p.buildings('townhall')[0]; const guards = p.units().filter((u) => u.type !== 'villager' && U.dist(u.x, u.y, th.x, th.y) < 16).length; for (let i = 0; i < 6 + guards; i++) { const e = Ent.unit('horseman', 0, th.x + 5 + (i % 6) * 0.4, th.y + 5 + Math.floor(i / 6) * 0.5); /* always outnumber the guard at home */ Game.units.push(e); Sim.setOrder(e, { type: 'attack', target: th }); } for (let i = 0; i < 80; i++) Game.tick(1 / 20); const rung = th.bell, inside = th.garrison.length; for (const e of Game.units) if (e.owner === 0 && e.type === 'horseman') Sim.kill(e, null); for (let i = 0; i < 400; i++) Game.tick(1 / 20); return { rung, inside, after: th.bell, out: p.units('villager').length, working: p.units('villager').filter((u) => u.order).length }; });
